    ...
    else
    {
        ...
        if ( it != dadGlobalList.end() )
        {
        	...
        	for (int i=0; i < ie->ipv6Data()->getNumAddresses(); i++ )
			{
				...
				EV << "[DBG] >>>> Assigned Address of  " << addr << endl;
			}
        	...
        	{
        		/*
        		 Neighbour *nce = neighbourCache.lookup(
        				 ie->ipv6Data()->getHomeAgentAddress(),
        				 ie->getInterfaceId());
				 */
        		 // The MN is on a new network and the HA is not reachable.
        		 //TODO: It would be great to have here another most performant algotithm for handover.
        		 // use link indication ????
        		 ...
        		 //}else {
        		 //	 EV << "DBG HA is still reachable not performing handover!" << endl;
        		 //}
		  		/*
